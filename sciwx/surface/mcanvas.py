...
class MCanvas3D(wx.Panel):
    def __init__( self, parent, meshset=None):
        ...
        self.canvas = Canvas3D(self, meshset)
        ...
        if meshset!=None: self.cho_obj.Set(list(meshset.objs.keys()))
    
    def set_mesh(self, meshset):
        self.canvas.set_mesh(meshset)
        self.cho_obj.Set(list(meshset.objs.keys()))
        

    @property
    def meshset(self): return self.canvas.meshset
    ...
    def get_obj(self, name):
        return self.canvas.meshset.get_obj(name)
    ...
    def add_surf_asyn(self, *p, **key):
        self.canvas.add_surf_asyn(*p, **key)
        self.cho_obj.Append(p[0])

    def add_surf(self, *p, **key):
        self.canvas.add_surf(*p, **key)
        self.cho_obj.Append(p[0])

    def add_mark_asyn(self, *p, **key):
        self.canvas.add_mark_asyn(*p, **key)
        self.cho_obj.Append(p[0])

    def add_mark(self, *p, **key):
        self.canvas.add_mark(*p, **key)
        self.cho_obj.Append(p[0])